...
# In CI the MRT deploy job has no SFCC credentials (dw.json is gitignored), and
# the cartridge deploy job is already pinned to a fixed code version via the
# SFCC_CODE_VERSION variable — so when we cannot query code versions, skip
# gracefully instead of failing the deploy.
LIST_JSON=$(b2c code list --json 2>/dev/null || true)
ACTIVE=$(printf '%s' "$LIST_JSON" | python3 -c "
import json, sys
try:
...
if [ -z "$ACTIVE" ]; then
    echo "Cannot query code versions (b2c CLI unavailable or not authenticated) — skipping SFRA cartridge check."
    exit 0
fi

HAS_SFRA=$(printf '%s' "$LIST_JSON" | python3 -c "
import json, sys
